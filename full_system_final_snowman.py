...
# centers
x_big, y_big = 0.0, 0.0
x_small, y_small = 0.0, R_big + R_small   # tangent on top
...
# compute per-point arc-length Δs (important)
# For circular pieces Δs ≈ R * dtheta; handle general case numerically by chord lengths
# easiest: compute chord distance between consecutive points (assume points in each piece are contiguous)
# if you concatenated separate loops, you may want to compute per-loop Δs; simple numeric:
coords = np.vstack([xib, yib]).T
# approximate ds as distance to previous point (circular so the two loops give internal consistent ds)
ds = np.sqrt(((np.roll(coords, -1, axis=0) - coords)**2).sum(axis=1))
# If the last point is not connected to the first (because we concatenated two circles),
# the ds at the junction will be large — optionally fix that by computing ds per-loop
# but for practical purposes you can also set Δs from radii*theta spacing:
# For safety, build Δs_per_point using R*dtheta where you know the indexes of each loop:
ds_big = R_big * dth_big * np.ones_like(theta_big)
ds_small = R_small * dth_small * np.ones_like(theta_small)
# after removing the tangent duplicates and potential reindexing, make a combined ds array similar to keep mask:
ds_combined = np.concatenate([ds_big, ds_small])
# use ds_combined (more stable than numerical chord at loop junctions)
delta_s = ds_combined.copy()
