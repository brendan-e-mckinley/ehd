...
yib = -(np.pi - 2 * delta_layer - rad) + rad * np.sin(theta) #-(np.pi + 2) + 
n_x = np.cos(theta)
n_y = np.sin(theta)
...
ld = loadmat('electrode_close_particle_rad_1.mat')
...
for t_step in range(N_t):
   ...
   # ------------------------------------------------------
    ...
    def plot_field(field, scalars_name, clim, save_path, Xint, Yint, X, Y, radius=1):
        ...
        disk = pv.Disc(center=(0, -(np.pi - 2 * delta_layer - rad), 0.001), inner=0, outer=radius, normal=(0, 0, 1), r_res=1, c_res=100) # -(np.pi + 2)
    ...
    fields = [
        (N_net, 'N_net', [-0.5, 0.5], f'img/electrode/n_net/neumann_close_rad_1.png'),
        (Np,    'N_p',   [0,  2], f'img/electrode/n_p/neumann_close_rad_1.png'),
        (Nm,    'N_m',   [0,  2], f'img/electrode/n_m/neumann_close_rad_1.png'),
    ]
...
# Save results
savemat('electrode_close_particle_rad_1.mat', {
    'ctxt_Rphi': ctxt,
    'u_next': u_next,
    'Xint': Xint,
    'Yint': Yint,
