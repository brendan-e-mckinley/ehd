...
# dynamic residual array
anderson_residuals = list()
fixed_point_residuals = list()
...
for t_step in range(start_t,N_t):
    ...
    u_next = G_u_n.copy()
    x_curr = G_u_n.copy()
    u_next_fps = G_u_n.copy()
    G_u_next = G_u_n.copy()
    err = []

    # FPS loop 
    for inner_its_fps in range(100000):
        schur_rhs = b_Op_Schur(t, x_curr, ctxt_BCs_Schur, ac_value, Np_prev, Nm_prev, U_fluid, V_fluid)
        rhs_computed = cpeo.schur_rhs_R(dLap, schur_rhs, Nx, Ny, Nib, delta_layer, Jop_prime)

        # Use SVD to solve
        p_sol = cpeo.solve_from_svd(U_schur, Sigma_schur, Vh_schur, rhs_computed)
        x_next = cpeo.post_processing_compute_R(dLap, p_sol, schur_rhs, Nx, Ny, Nib, delta_layer, Sop_prime)

        # Extract solution components
        Phi   = x_next[:Ny*Nx].reshape((Ny, Nx), order='F')
        Np    = x_next[Ny*Nx:2*Nx*Ny].reshape((Ny, Nx), order='F')
        Nm    = x_next[2*Ny*Nx:3*Nx*Ny].reshape((Ny, Nx), order='F')
        p     = x_next[3*Nx*Ny:3*Nx*Ny+Nib]
        p_p   = x_next[3*Nx*Ny+Nib:3*Nx*Ny+2*Nib]
        p_m   = x_next[3*Nx*Ny+2*Nib:]
        p_next = x_next[3*Nx*Ny:]

        # Check convergence
        res1, res2, res3 = cpeo.apply_Schur_R(dLap, [p, p_p, p_m], delta_layer, Nx, Ny, Sop_prime, Jop_prime)

        schur_rhs_next = b_Op_Schur(t, x_next, ctxt_BCs_Schur, ac_value, Np_prev, Nm_prev, U_fluid, V_fluid)
        rhs_computed_next = cpeo.schur_rhs_R(dLap, schur_rhs_next, Nx, Ny, Nib, delta_layer, Jop_prime)

        schur_residual = np.concatenate([res1, res2, res3])
        fp_err = np.linalg.norm(schur_residual - rhs_computed_next) / np.linalg.norm(rhs_computed_next)

        print(f'Iteration {inner_its_fps}: residual = {fp_err}')

        fixed_point_residuals.append(fp_err)

        if fp_err < 1e-5:
            break

        # Update iterate for next step
        x_curr = x_next.copy()

    # Anderson acceleration loop
    for inner_its in range(len(fixed_point_residuals)):
        ...
        anderson_residuals.append(err_curr)

fixed = np.array(fixed_point_residuals)
anderson = np.array(anderson_residuals)

plt.plot(len(fixed), fixed)
plt.plot(len(anderson), anderson)
plt.show()
