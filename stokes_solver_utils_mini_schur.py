...
def apply_A_Mini(x, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V):
    U = x[:N_U]
    offset = N_U
    V = x[offset:offset + N_V]
    offset = offset + N_V
    ...
    res = np.zeros_like(x)
    res[0:N_U] = Lap_U @ U - G_x @ Pi
    offset = N_U
    res[offset:offset + N_V] = Lap_V @ V - G_y @ Pi
    offset = offset + N_V
    res[offset:] = D_x @ U + D_y @ V
    
    return res

def SchurLinearOperator(shape, UGridX, UGridY, VGridX, VGridY, xib, yib, delta_x, delta_y, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, Nib, tol, cut):
    n = shape
    def mv(unknowns):
        unknown_Lam_x = unknowns[:Nib]
        unknown_Lam_y = unknowns[Nib:]
        return apply_Schur([unknown_Lam_x, unknown_Lam_y], UGridX, UGridY, VGridX, VGridY, xib, yib, delta_x, delta_y, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, tol, cut)
    return LinearOperator((n, n), matvec=mv)

# def MiniSchurLinearOperator(shape, Lap_U, Lap_V, G_x, G_y, D_x, D_y):
#     n = shape
#     def mv(Pi):
#         return apply_Schur_Mini(Pi, Lap_U, Lap_V, G_x, G_y, D_x, D_y)
#     return LinearOperator((n, n), matvec=mv)

def MiniAxLinearOperator(shape, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V):
    n = shape
    def mv(x):
        return apply_A_Mini(x, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V)
    return LinearOperator((n, n), matvec=mv)

# def apply_Ainv_Mini(Lap_U, Lap_V, target_vec):
...
# def apply_Schur_Mini(Pi, Lap_U, Lap_V, G_x, G_y, D_x, D_y):
#     # apply B 
#     B_y_1 = -G_x @ Pi
#     B_y_2 = -G_y @ Pi

#     # apply A inverse 
#     Ainv_B_y_1, Ainv_B_y_2 = apply_Ainv_Mini(Lap_U, Lap_V, [B_y_1, B_y_2])

#     # apply C 
#     res = D_x @ Ainv_B_y_1 + D_y @ Ainv_B_y_2

#     return res

def apply_Ainv(Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, tol, target_vec):
    # TODO: schur? 
    # B_y_1 = -G_x @ target_vec_1
    # B_y_2 = -G_y @ target_vec_2

    # # build Schur RHS
    # RHS_schur_mini = schur_rhs_mini()
    
    # solve system
    shape = N_U + N_V + N_P
    AxOp = MiniAxLinearOperator(shape, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V)
    sol, info = gmres(AxOp, target_vec, rtol=tol, restart=500, callback=lambda rk: print(f"GMRES residual: {np.linalg.norm(rk)}"))

    result_vec_1 = sol[:N_U]
    result_vec_2 = sol[N_U:N_U + N_V]
    result_vec_3 = sol[N_U + N_V:]

    return [result_vec_1, result_vec_2, result_vec_3]

def apply_Schur(unknown_blocks, UGridX, UGridY, VGridX, VGridY, xib, yib, delta_x, delta_y, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, tol, cut):
    Lam_x, Lam_y = unknown_blocks

    # apply B 
    B_y_1 = spreadQ_x(UGridX, UGridY, xib, yib, Lam_x, delta_x, cut) 
    B_y_2 = spreadQ_y(VGridX, VGridY, xib, yib, Lam_y, delta_y, cut)
    B_y_3 = np.zeros(N_P)

    # apply A inverse 
    Ainv_B_y_1, Ainv_B_y_2, Ainv_B_y_3 = apply_Ainv(Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, tol, np.concatenate([B_y_1, B_y_2, B_y_3]))

    # apply C 
    res_1 = interpPhi_x(UGridX, UGridY, xib, yib, Ainv_B_y_1, delta_x, cut)
    res_2 = interpPhi_y(VGridX, VGridY, xib, yib, Ainv_B_y_2, delta_y, cut)
...
def schur_rhs(rhs, Lap_U, Lap_V, G_x, G_y, D_x, D_y, delta_x, delta_y, UGridX, UGridY, VGridX, VGridY, xib, yib, N_U, N_V, N_P, Nib, tol, cut):
    rhs_1 = rhs[0:N_U]
    offset = N_U
    ...
    Ainv_U, Ainv_V, Ainv_P = apply_Ainv(Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, tol, np.concatenate([rhs_1, rhs_2, rhs_3]))
    CAinv_1 = interpPhi_x(UGridX, UGridY, xib, yib, Ainv_U, delta_x, cut)
    CAinv_2 = interpPhi_y(VGridX, VGridY, xib, yib, Ainv_V, delta_y, cut)

    schur_rhs_1 = CAinv_1 - rhs_4
    schur_rhs_2 = CAinv_2 - rhs_5

    return np.concatenate((schur_rhs_1, schur_rhs_2))

# def schur_rhs_mini(rhs, Lap_U, Lap_V, D_x, D_y):
#     rhs_1 = rhs[0:N_U]
#     offset = N_U
#     rhs_2 = rhs[offset:offset + N_V]
#     offset = offset + N_V
...
#     Ainv_u_1, Ainv_u_2 = apply_Ainv_Mini(Lap_U, Lap_V, [rhs_1, rhs_2])
#     CAinv = D_x @ Ainv_u_1 + D_y @ Ainv_u_2

#     schur_rhs = CAinv - rhs_3

#     return np.concatenate(schur_rhs)
...
def solve(left_bound, right_bound, f_bc, g_bc, h_bc, z_x, z_y, rad, Nx, tol, cut):
    ...
    RHS_schur = schur_rhs(RHS, Lap_U, Lap_V, G_x, G_y, D_x, D_y, delta_x, delta_y, UGridX, UGridY, VGridX, VGridY, xib, yib, N_U, N_V, N_P, Nib, tol, cut)

    # Solve
    shape = 2 * Nib
    SchurOp = SchurLinearOperator(shape, UGridX, UGridY, VGridX, VGridY, xib, yib, delta_x, delta_y, Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, Nib, tol, cut)
    ...
    # Postprocessing: compute U, V, and P
    postprocessing_rhs_1 = f_bc - spreadQ_x(UGridX, UGridY, xib, yib, lam_X, delta_x, cut)
    postprocessing_rhs_2 = g_bc - spreadQ_x(VGridX, VGridY, xib, yib, lam_Y, delta_y, cut)
    postprocessing_rhs_3 = h_bc
    U, V, P = apply_Ainv(Lap_U, Lap_V, G_x, G_y, D_x, D_y, N_U, N_V, N_P, tol, np.concatenate([postprocessing_rhs_1, postprocessing_rhs_2, postprocessing_rhs_3]))

    # U = compute_U_postprocessing(lam_X, UGridX, UGridY, xib, yib, Lap_U, G_x, delta_x, f_bc, cut)
    # V = compute_V_postprocessing(Plam_Y, VGridX, VGridY, xib, yib, Lap_V, G_y, delta_y, g_bc, cut)
...
# def compute_U(xx, yy):
#     # zero on boundaries
#     return np.sin(2 * np.pi * yy) * (np.cos(2 * np.pi * xx) - 1)

# def compute_V(xx, yy):
#     # zero on boundaries
#     return np.sin(2 * np.pi * xx) * (-np.cos(2 * np.pi * yy) + 1)

# def compute_Lap_U(xx, yy):
#     # ∇²U = d²U/dx² + d²U/dy²
#     term_xx = - (2 * np.pi)**2 * np.sin(2 * np.pi * yy) * np.cos(2 * np.pi * xx)
#     term_yy = - (2 * np.pi)**2 * np.sin(2 * np.pi * yy) * (np.cos(2 * np.pi * xx) - 1)
#     return term_xx + term_yy

# def compute_Lap_V(xx, yy):
#     # ∇²V = d²V/dx² + d²V/dy²
#     term_xx = - (2 * np.pi)**2 * np.sin(2 * np.pi * xx) * (-np.cos(2 * np.pi * yy) + 1)
#     term_yy = (2 * np.pi)**2 * np.sin(2 * np.pi * xx) * np.cos(2 * np.pi * yy)
#     return term_xx + term_yy

# def compute_Dx_U(xx, yy):
#     return -2 * np.pi * np.sin(2 * np.pi * yy) * np.sin(2 * np.pi * xx)

# def compute_Dy_V(xx, yy):
#     return 2 * np.pi * np.sin(2 * np.pi * xx) * np.sin(2 * np.pi * yy)
...
# def compute_f(xx, yy):
#     return compute_Lap_U(xx, yy) - (2 * np.pi * np.cos(2 * np.pi * xx) * np.sin(2 * np.pi * yy))

# def compute_g(xx, yy):
#     return compute_Lap_V(xx, yy) - (2 * np.pi * np.sin(2 * np.pi * xx) * np.cos(2 * np.pi * yy))
...
#     z_x[:] = interpPhi_x(UGridX, UGridY, xib, yib, UExact, delta_x, cut)
#     z_y[:] = interpPhi_y(VGridX, VGridY, xib, yib, VExact, delta_y, cut)
...
#     f_bc = f.ravel(order='F') + spreadQ_x(UGridX, UGridY, xib, yib, lam_x_exact, delta_x, cut) + f_bc_mat.ravel(order='F')
...
#     g_bc_mat[0, :] = 0.0
#     g_bc_mat[-1, :] = 0.0
#     g_bc = g.ravel(order='F') + spreadQ_y(VGridX, VGridY, xib, yib, lam_y_exact, delta_y, cut) + g_bc_mat.ravel(order='F')
...
#     U, V, P, lam_X, lam_Y = solve(0, L, f_bc, g_bc, h_bc, z_x, z_y, rad, Nx, tol, cut)
