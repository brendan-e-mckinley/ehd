...
def Grad_dot_Grad_neumann_far_field(Phi, N_pm, dx, dy, Nx, Ny, E_0, electrode):
    ...
    Phi_y[-1, :]    = (Phi[-1, :] + dy * E_0 - Phi[-2, :])     * (0.5 / dy)  # Neumann = E_0
...
def Grad_dot_Grad_neumann(Phi, N_pm, dx, dy, Nx, Ny, E_0, electrode):
    ...
    N_pm_y[1:-1, :] = (N_pm[2:, :] - N_pm[:-2, :]) * (0.5 / dy)  # central
    N_pm_y[0, :]    = (N_pm[1, :]  - electrode[:])  * (0.5 / dy)  # Nonlinear Dirichlet
    N_pm_y[-1, :]   = (N_pm[-1,:] - N_pm[-2, :])    * (0.5 / dy)  # Neumann = 0
...
def Constrained_Lap_no_particle(ctxt, dLap, delta_layer, Nx, Ny):
    ...
    A_x_Ctx[:sz] = dl2 * Phi - dLap.solve_A(0.5*N_p - 0.5*N_m)
    A_x_Ctx[sz:2*sz] = N_p
...
def Build_RHS_rho_neumann(ctxt, electrode_p, electrode_m, ctxt_BCs, U, V, dLap_phi, dLap_npm, Lap_phi, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, Nib, Jop, Jop_prime, dx, beta_BC):
    b_Ctx = np.zeros_like(ctxt_BCs)
    ...
    Phi_full[1:-1, 1:-1] = Phi_reshaped
    Phi_full[1:-1, 0] = Phi_reshaped[:, 0]
    Phi_full[1:-1, -1] = Phi_reshaped[:, -1]
    Phi_full[-1, 1:-1] = dx * beta_BC + Phi_reshaped[-1, :]

    # new, maybe wrong
    Phi_full[-1, 0] = dx * beta_BC + Phi_reshaped[-1, 0]
    Phi_full[-1, -1] = dx * beta_BC + Phi_reshaped[-1, -1]

    N_p_full[1:-1, 1:-1] = N_p_reshaped
    N_m_full[1:-1, 1:-1] = N_m_reshaped

    # new, maybe wrong 
    N_p_full[1:-1, 0] = N_p_reshaped[:, 0]
    N_p_full[1:-1, -1] = N_p_reshaped[:, -1]
    N_m_full[1:-1, 0] = N_m_reshaped[:, 0]
    N_m_full[1:-1, -1] = N_m_reshaped[:, -1]

    N_p_full[0, 1:-1] = electrode_p
    N_m_full[0, 1:-1] = electrode_m

    N_p_full[0, 0] = N_p_full[0, 1]
    N_p_full[0, -1] = N_p_full[0, -2]
    N_m_full[0, 0] = N_m_full[0, 1]
    N_m_full[0, -1] = N_m_full[0, -2]
    ...
    U = U.reshape(Ny + 1, Nx + 2, order='F')
    V = V.reshape(Ny + 2, Nx + 1, order='F')

    U_at_N = 0.5 * (U[:-1, 1:-1] + U[1:, 1:-1])   # shape (Ny, Nx)
    V_at_N = 0.5 * (V[1:-1, :-1] + V[1:-1, 1:])   # shape (Ny, Nx)
    ...
    adv_p = alpha_p * (U_at_N * dNdx_p + V_at_N * dNdy_p)
    adv_m = alpha_m * (U_at_N * dNdx_m + V_at_N * dNdy_m)
...
def Build_RHS_Schur_System_neumann(ctxt, electrode_p, electrode_m, ctxt_BCs, U, V, Lap_phi, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, Nib, Jop, Jop_prime, dx, beta_BC):
    b_Ctx = np.zeros_like(ctxt_BCs)
...
def AxOpLinearOperatorNoParticle(dLap, shape, Nx, Ny, delta_layer):
    n = shape
    def mv(ctxt):
        res = Constrained_Lap_no_particle(ctxt, dLap, delta_layer, Nx, Ny)
        return res
    return LinearOperator((n, n), matvec=mv)
