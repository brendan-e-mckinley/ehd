...
def Build_RHS_rho(ctxt, ctxt_BCs, N_p_prev, N_m_prev, U, V, Lap, dLap, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, Nib, Jop, Jop_prime, dx, dt):
    ...
    b_Ctx[sz:2*sz] =  -dLap.solve_A(-N_p * computed_lap + adv_p.ravel(order='F') - N_p_BC - G_d_G_p(Phi, N_p) + n_p_time_deriv)
    b_Ctx[2*sz:3*sz] =  -dLap.solve_A(N_m * computed_lap + adv_m.ravel(order='F') - N_m_BC + G_d_G_m(Phi, N_m) + n_m_time_deriv)
...
def Build_RHS_Schur_System(ctxt, ctxt_BCs, N_p_prev, N_m_prev, U, V, Lap, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, Nib, Jop, Jop_prime, dx, dt):
    b_Ctx = np.zeros_like(ctxt_BCs)
    ...
    b_Ctx[:sz] =  -dl2 * Phi_BC
    b_Ctx[sz:2*sz] =  -N_p * computed_lap + adv_p.ravel(order='F') - N_p_BC - G_d_G_p(Phi, N_p) + n_p_time_deriv
    b_Ctx[2*sz:3*sz] =  N_m * computed_lap + adv_m.ravel(order='F') - N_m_BC + G_d_G_m(Phi, N_m) + n_m_time_deriv
    b_Ctx[q_i:q_i+Nib] = Q_BC
    b_Ctx[q_i+Nib:q_i+2*Nib] = Q_p_BC - Jop(N_p.reshape(Ny, Nx, order='F')) * Jop_prime(Phi.reshape(Ny, Nx, order='F'))
    b_Ctx[q_i+2*Nib:q_i+3*Nib] = Q_m_BC + Jop(N_m.reshape(Ny, Nx, order='F')) * Jop_prime(Phi.reshape(Ny, Nx, order='F'))
