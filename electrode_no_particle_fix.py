...
x_coarse = np.linspace(-L/2, L/2, 64+2) 
...
beta_BC = 50 / L
# sigma_bc = 0.78  # 0.68
delta_layer = 5 * dx_coarse  # 5*dx; %6*dx;
...
Np_BCs[0,:] += (1/dx/dx) * Np_electrode(Phi_BC[1,1:-1], N_p_BC[0,1:-1], N_p_BC[1,1:-1], N_p_BC[2,1:-1])
...
## Initial guesses for Rphi = rho system
ld = loadmat('electrode_no_particle.mat')
...
Phi_ld = ctxt_ld[:sz_ld].reshape(Ny_ld, Nx_ld, order='F')
N_p_ld = ctxt_ld[sz_ld:2*sz_ld].reshape(Ny_ld, Nx_ld, order='F')
N_m_ld = ctxt_ld[2*sz_ld:3*sz_ld].reshape(Ny_ld, Nx_ld, order='F')
...
def AxOp_no_particle(x):
    return cpeo.Constrained_Lap_no_particle(x, dLap, delta_layer, Nx, Ny)

def b_Op_no_particle(x, bcs, phibc, npbc, nmbc, U_fluid, V_fluid):
    return cpeo.Build_RHS_rho_no_particle(x, bcs, phibc, npbc, nmbc, U_fluid, V_fluid, Lap, dLap, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, dx)
...
shape = 3 * Nx * Ny
AxOperator = cpeo.AxOpLinearOperatorNoParticle(dLap, shape, Nx, Ny, delta_layer)

computedRHS = b_Op_no_particle(ctxt, ctxt_BCs, Phi_BC, N_p_BC, N_m_BC, U_fluid, V_fluid)
...
for t_step in range(N_t):
    ...
    #######################################
    computedRHS = b_Op_no_particle(u_next, ctxt_BCs, Phi_BC, N_p_BC, N_m_BC, U_fluid, V_fluid)
    G_u_n, _ = gmres(AxOperator, computedRHS, rtol=tol, restart=500)#, callback=lambda rk: print(f"GMRES residual: {np.linalg.norm(rk)}"))
    ...
    for inner_its in range(100000):      
        ...
        # Check convergence
        res_next = cpeo.Constrained_Lap_no_particle(u_next, dLap, delta_layer, Nx, Ny)
...
# Save results
savemat('electrode_no_particle.mat', {
    'ctxt_Rphi': ctxt,
    'u_next': u_next,
    'Xint': Xint,
    'Yint': Yint,
...
def plot_field(field, scalars_name, clim, save_path, Xint, Yint):
    """
    ...
    x_min, x_max = Xint.min(), Xint.max()
    y_min, y_max = Yint.min(), Yint.max()
...
fields = [
    (N_net, 'N_net', [-0.5, 0.5], f'img/electrode/n_net/no_particle_fix.png'),
    (Np,    'N_p',   [0,  2], f'img/electrode/n_p/no_particle_fix.png'),
    (Nm,    'N_m',   [0,  2], f'img/electrode/n_m/no_particle_fix.png'),
]

for field, name, clim, path in fields:
    plot_field(field, name, clim, path, Xint, Yint)
...
# fig = plt.figure()
# ax = fig.add_subplot(111, projection="3d")
# ax.plot_surface(Xint, Yint, ((Np - Nm) / 2), cmap=cmap, edgecolor='none')
# ax.set_title("N_net")
# ax.set_xlabel("x"); ax.set_ylabel("y")
