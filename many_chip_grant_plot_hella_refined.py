...
xib, yib, n_x, n_y = chip.get_c_d_c()
chip_b, chip_t, d = chip.make_chip_disc_chip()
x_t = chip_t['x']
y_t = chip_t['y']
x_b = chip_b['x']
y_b = chip_b['y']
...
ld = loadmat('chip_results_hella_refined.mat')
...
# savemat('chip_results_hella_refined.mat', {
#             'ctxt_Rphi': ctxt,
#             'u_next': u_next,
#             'Xint': Xint,
#             'Yint': Yint,
...
# Build boundary path for disc
boundary_d = np.column_stack([x_d, y_d])
boundary_d = np.vstack([boundary_d, boundary_d[0]])
path_d = Path(boundary_d)
mask_d = path_d.contains_points(points).reshape(X.shape)
...
patch_top = PathPatch(path_top, facecolor='gray', edgecolor='black', alpha=0.5)
patch_bot = PathPatch(path_bot, facecolor='gray', edgecolor='black', alpha=0.5)
patch_d = PathPatch(path_d, facecolor='gray', edgecolor='black', alpha=0.5)

# Apply to velocity fields
U_masked = np.where(mask_top | mask_bot | mask_d, np.nan, UFull)
V_masked = np.where(mask_top | mask_bot | mask_d, np.nan, VFull)

## INT MASK
# Build boundary path for top region
boundary_top_int = np.column_stack([x_t, y_t])
boundary_top_int = np.vstack([boundary_top_int, boundary_top_int[0]])
path_top_int = Path(boundary_top_int)

# Test which (Xint, Yint) grid points are inside
points_int = np.column_stack([Xint.ravel(), Yint.ravel()])
mask_top_int = path_top_int.contains_points(points_int).reshape(Xint.shape)

# Build boundary path for disc
boundary_d_int = np.column_stack([x_d, y_d])
boundary_d_int = np.vstack([boundary_d_int, boundary_d_int[0]])
path_d_int = Path(boundary_d_int)
mask_d_int = path_d_int.contains_points(points_int).reshape(Xint.shape)

# Same for bottom region
boundary_bot_int = np.column_stack([x_b, y_b])
boundary_bot_int = np.vstack([boundary_bot_int, boundary_bot_int[0]])
path_bot_int = Path(boundary_bot_int)
mask_bot_int = path_bot_int.contains_points(points_int).reshape(Xint.shape)

patch_top_int = PathPatch(path_top_int, facecolor='gray', edgecolor='black', alpha=0.5)
patch_bot_int = PathPatch(path_bot_int, facecolor='gray', edgecolor='black', alpha=0.5)
patch_d_int = PathPatch(path_d_int, facecolor='gray', edgecolor='black', alpha=0.5)

# Apply to velocity fields
Np_masked = np.where(mask_top_int | mask_bot_int | mask_d_int, np.nan, Np)
Phi_masked = np.where(mask_top_int | mask_bot_int | mask_d_int, np.nan, Phi)
Phi_dist_check = Phi_exact(Xint, Yint)
Phi_dist_masked = np.where(mask_top_int | mask_bot_int | mask_d_int, np.nan, Phi_dist_check)
...
# cmr.viola trimmed colormap
deep = plt.cm.get_cmap('cmo.deep', 256)
...
left_half  = Xint < 0   # for the fine grid (ion conc + E-field)
right_half = X    >= 0  # for the coarse grid (velocity)

# ── LEFT HALF: ion concentration ──────────────────────────────────────────────

Np_left = np.where(left_half, Np_masked, np.nan)

c_left = ax.pcolormesh(
    Xint, Yint, Np_left,
    cmap='RdBu_r', shading='auto',
    vmin=0.98, vmax=1.02, zorder=1
)
fig.colorbar(c_left, ax=ax, location='left', label=r'$N_p$', pad=0.12)

# E-field streamlines on left half only (dashed, black)

dPhi_dy, dPhi_dx = np.gradient(Phi_dist, Yint[:, 0], Xint[0, :])
dPhi_dx_left = np.where(left_half, dPhi_dx, np.nan)
dPhi_dy_left = np.where(left_half, dPhi_dy, np.nan)

# # Step 1: draw streamlines with density (no start_points) for uniform coverage
# # suppress default arrows by setting arrowsize=0
# sp = ax.streamplot(
#     Xint, Yint, dPhi_dx_left, dPhi_dy_left,
#     color='black', linewidth=0.8, arrowsize=0,
#     density=2, zorder=3
# )

# # Step 2: manually place arrows at distributed points along the lines
# from matplotlib.patches import FancyArrowPatch

# # choose a few y-levels where you want arrows to appear
# arrow_y_levels = np.linspace(-1.5, 1.5, 6)  # tune count and spacing to taste

# for line in sp.lines.get_paths():
#     verts = line.vertices
#     if len(verts) < 4:
#         continue
#     for y_target in arrow_y_levels:
#         # find the index where the line crosses this y-level
#         diffs = np.abs(verts[:, 1] - y_target)
#         i = np.argmin(diffs)
#         # only place arrow if the line actually passes near this level
#         if diffs[i] > 0.15 or i >= len(verts) - 1:
#             continue
#         x0, y0 = verts[i]
#         x1, y1 = verts[i+1]
...
for line in sp.lines.get_paths():
    ...
    if len(verts) < 4:
        continue
    # sample arrows at several evenly-spaced positions along each streamline
    indices = np.linspace(len(verts)//4, len(verts)-2, 3).astype(int)
    for i in indices:
        x0, y0 = verts[i]
        x1, y1 = verts[i+1]
        ax.annotate('', xy=(x1, y1), xytext=(x0, y0),
...
# # regular grid
# start_x = np.linspace(-1.9, -0.1, 10)
# start_y = np.linspace(-1.8,  1.8, 16)
# sx, sy = np.meshgrid(start_x, start_y)

# # add small random offsets
# sx += rng.uniform(-0.1, 0.1, sx.shape)
# sy += rng.uniform(-0.1, 0.1, sy.shape)

# start_points = np.column_stack([sx.ravel(), sy.ravel()])

# # filter out any seeds inside the masked geometry
# valid = ~(
#     path_top_int.contains_points(start_points) |
#     path_bot_int.contains_points(start_points) |
#     path_d_int.contains_points(start_points)
# )
...
#     color='black', linewidth=0.8, arrowsize=1.2, zorder=3,
#     minlength=0.1,   # allow shorter lines → more arrows in tight regions
#     maxlength=0.8,   # shorter max length → arrows placed sooner along each line
# )

for col in ax.collections:
    col.set_linestyle('dashed')

# ── RIGHT HALF: velocity magnitude + flow streamlines ─────────────────────────

speed       = np.sqrt(U_masked**2 + V_masked**2)
speed_right = np.where(right_half, speed,    np.nan)
U_right     = np.where(right_half, U_masked, np.nan)
V_right     = np.where(right_half, V_masked, np.nan)

plasma_full = plt.cm.get_cmap('plasma', 256)
plasma_top  = ListedColormap(plasma_full(np.linspace(0.5, 1.0, 256)))

c_right = ax.pcolormesh(
    X, Y, speed_right,
    cmap=custom_cmap, shading='auto', zorder=1
)
fig.colorbar(c_right, ax=ax, location='right', label='|velocity|', pad=0.02)

# snapshot which collections exist before streamplot
n_cols_before = len(ax.collections)
...
# ── dividing line ─────────────────────────────────────────────────────────────
ax.axvline(x=0, color='white', linewidth=1.2, linestyle='--', zorder=4, alpha=0.6)

# ── patches (chip + disk) ─────────────────────────────────────────────────────
kw_chip = dict(facecolor='gray',  edgecolor='black', alpha=0.7, zorder=5)
kw_disk = dict(facecolor='black', edgecolor='black', alpha=0.9, zorder=5)

ax.add_patch(PathPatch(path_top_int, **kw_chip))
ax.add_patch(PathPatch(path_bot_int, **kw_chip))
ax.add_patch(PathPatch(path_d_int,   **kw_disk))
...
# def make_patches(path_top, path_bot, path_d, disk_color='black'):
#     """Return fresh PathPatch objects (each can only be added to one axes)."""
#     kw_chip = dict(facecolor='gray', edgecolor='black', alpha=0.7, zorder=5)
#     kw_disk = dict(facecolor=disk_color, edgecolor='black', alpha=0.9, zorder=5)
#     return (
#         PathPatch(path_top, **kw_chip),
#         PathPatch(path_bot, **kw_chip),
#         PathPatch(path_d,   **kw_disk),
#     )

# # ── LEFT: ion concentration + E-field lines ──────────────────────────────────
...
# c_left = ax_left.pcolormesh(
#     Xint, Yint, Np_masked,
#     cmap=custom_cmap, shading='auto',
#     vmin=0.975, vmax=1.125, zorder=1
# )
# fig.colorbar(c_left, ax=ax_left, label=r'$N_p$')

# # E-field streamlines (dashed, black)
# dPhi_dy, dPhi_dx = np.gradient(Phi_masked, Yint[:, 0], Xint[0, :])
# ax_left.streamplot(
#     Xint, Yint, dPhi_dx, dPhi_dy,
#     color='black', density=3, linewidth=0.8, arrowsize=1.2,
#     zorder=3,
#     # matplotlib streamplot doesn't accept linestyle directly;
#     # grab the lines collection and restyle after the call
# )
# # make the stream lines dashed
# for col in ax_left.collections:
#     col.set_linestyle('dashed')

# pt, pb, pd = make_patches(path_top_int, path_bot_int, path_d_int, disk_color='black')
# ax_left.add_patch(pt); ax_left.add_patch(pb); ax_left.add_patch(pd)

# ax_left.set_xlim(-2, 2); ax_left.set_ylim(-2, 2)
# ax_left.set_xlabel('X'); ax_left.set_ylabel('Y')
# ax_left.set_title('Ion concentration & E-field lines')
# ax_left.set_aspect('equal')
# ax_left.grid(True, linewidth=0.4, alpha=0.5)

# # ── RIGHT: velocity magnitude + flow streamlines ──────────────────────────────

# speed = np.sqrt(U_masked**2 + V_masked**2)

# c_right = ax_right.pcolormesh(
#     X, Y, speed,
#     cmap='plasma', shading='auto', zorder=1
# )
# fig.colorbar(c_right, ax=ax_right, label='|velocity|')
...
# pt2, pb2, pd2 = make_patches(path_top, path_bot, path_d, disk_color='black')
# ax_right.add_patch(pt2); ax_right.add_patch(pb2); ax_right.add_patch(pd2)

# ax_right.set_xlim(-2, 2); ax_right.set_ylim(-2, 2)
# ax_right.set_xlabel('X'); ax_right.set_ylabel('Y')
# ax_right.set_title('Velocity magnitude & flow lines')
...
# fig, ax = plt.subplots()
# c = ax.pcolormesh(Xint, Yint, Phi - Phi_dist_check, cmap='viridis', shading='auto')
# fig.colorbar(c, ax=ax)
# ax.set_xlabel('X-coordinate')
# ax.set_ylabel('Y-coordinate')
# ax.set_title('Disturbance')
# ax.grid(True)
# ax.add_patch(PathPatch(path_top_int, facecolor='gray', edgecolor='black', alpha=0.5))
# ax.add_patch(PathPatch(path_bot_int, facecolor='gray', edgecolor='black', alpha=0.5))

# fig, ax = plt.subplots()
# c = ax.pcolormesh(Xint, Yint, Np_masked, cmap=custom_cmap, shading='auto', vmin=0.9, vmax=1.1)
# fig.colorbar(c, ax=ax)
# ax.set_xlabel('X-coordinate')
# ax.set_ylabel('Y-coordinate')
# ax.set_title('N_p')
...
# plt.figure(figsize=(8, 6))
# fig, ax = plt.subplots()
# ax.streamplot(X, Y, U_masked, V_masked, color='red', density=5, linewidth=1, arrowsize=1.5)
# ax.set_xlabel('X-coordinate')
# ax.set_ylabel('Y-coordinate')
# ax.set_title('Flow Field Streamline Plot')
# ax.set_xlim(-2, 2)
# ax.set_ylim(-2, 2)
# ax.grid(True)
...
# dPhi_dy, dPhi_dx = np.gradient(Phi, Yint[:, 0], Xint[0, :])

# fig, ax = plt.subplots(figsize=(8, 6))
# ax.streamplot(Xint, Yint, dPhi_dx, dPhi_dy, color='blue', density=5, linewidth=1, arrowsize=1.5)
# ax.set_xlabel('X-coordinate')
# ax.set_ylabel('Y-coordinate')
# ax.set_title('grad(Phi) Field Lines')
# ax.set_xlim(-2, 2)
# ax.set_ylim(-2, 2)
# ax.grid(True)
# ax.add_patch(PathPatch(path_top, facecolor='gray', edgecolor='black', alpha=0.5))
# ax.add_patch(PathPatch(path_bot, facecolor='gray', edgecolor='black', alpha=0.5))
# ax.add_patch(PathPatch(path_d_int, facecolor='gray', edgecolor='black', alpha=0.5))

# fig, ax = plt.subplots()
# c = ax.pcolormesh(Xint, Yint, np.sqrt(dPhi_dx**2 + dPhi_dy**2), cmap='viridis', shading='auto')
# fig.colorbar(c, ax=ax)
# ax.set_xlabel('X-coordinate')
# ax.set_ylabel('Y-coordinate')
# ax.set_title('|E|')
...
# fig, ax = plt.subplots()
# c = ax.pcolormesh(Xint, Yint, dPhi_dy, cmap='viridis', shading='auto')
# fig.colorbar(c, ax=ax)
# ax.set_xlabel('X-coordinate')
# ax.set_ylabel('Y-coordinate')
# ax.set_title('E_y')
