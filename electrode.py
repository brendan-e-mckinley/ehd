...
for t_step in range(N_t):
    ...
    sm = plt.cm.ScalarMappable(cmap=custom_cmap, norm=Normalize(vmin=-0.8, vmax=0.8))
    sm.set_array([])
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'img/n_net_dc_CPEO/n_net_{t_step}.png', dpi=100, bbox_inches='tight', facecolor='white')
    plt.close()
