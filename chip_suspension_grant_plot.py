...
for chip_inst in chips:
    x_specific.append(chip_inst['x'])
    y_specific.append(chip_inst['y'])
for disc in discs:
    x_specific.append(disc['x'])
    y_specific.append(disc['y'])
...
ld = loadmat('chip_suspension_close.mat')
...
savemat('chip_suspension_close_hella_refined.mat', {
            'ctxt_Rphi': ctxt,
            'u_next': u_next,
            'Xint': Xint,
            'Yint': Yint,
...
for it, chip in enumerate(chips):
    boundary = np.column_stack([x_specific[it], y_specific[it]])
...
ax.grid(True)
for path in paths:
    ax.add_patch(PathPatch(path, facecolor='gray', edgecolor='black', alpha=0.5))
