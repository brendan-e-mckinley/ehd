...
x_coarse = np.linspace(-L/2, L/2, 128+2) 
...
def b_Op_no_particle(x, bcs, U_fluid, V_fluid):
    return cpeo.Build_RHS_rho_no_particle(x, bcs, U_fluid, V_fluid, Lap, dLap, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, dx)
...
computedRHS = b_Op_no_particle(ctxt, ctxt_BCs, U_fluid, V_fluid)
...
for t_step in range(N_t):
    ...
    #######################################
    computedRHS = b_Op_no_particle(u_next, ctxt_BCs, U_fluid, V_fluid)
    G_u_n, _ = gmres(AxOperator, computedRHS, rtol=tol, restart=500)#, callback=lambda rk: print(f"GMRES residual: {np.linalg.norm(rk)}"))
...
fields = [
    (N_net, 'N_net', [-0.5, 0.5], f'img/electrode/n_net/no_particle.png'),
    (Np,    'N_p',   [0,  2], f'img/electrode/n_p/no_particle.png'),
    (Nm,    'N_m',   [0,  2], f'img/electrode/n_m/no_particle.png'),
]
