...
ld = loadmat('electrode_close_particle_large_domain_rad_1_stokes.mat')
...
def b_Op_Schur(ctxt, bcs, U_fluid, V_fluid): 
    return cpeo.Build_RHS_Schur_System_neumann(ctxt, bcs, U_fluid, V_fluid, dLap_phi, dLap_npm, Lap_phi, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, Nib, Jop, Jop_prime, dx)
...
Np_full[1:-1, 1:-1] = Np
Nm_full[1:-1, 1:-1] = Nm
Np_full[0, 1:-1] = Np_electrode(Phi[0,:], Np[0,:])
Nm_full[0, 1:-1] = Nm_electrode(Phi[0,:], Nm[0,:])
...
U, V, P, lam_X, lam_Y = stokes.solve_factorized(-L/2, L/2, stokes_LU, U_schur_fluid, Sigma_schur_fluid, Vh_schur_fluid, f_bc, g_bc, h_bc, z_x, z_y, xib, yib, Nib, Nx, tol, cut)
...
for t_step in range(N_t):
   ...
   # ------------------------------------------------------
    ...
    def plot_field(field, scalars_name, clim, save_path, Xint, Yint, X, Y, UFull, VFull, radius=1):
        ...
        U_masked = np.where(X**2 + (Y + 2 * np.pi - 2 * delta_layer - rad)**2 <= radius**2, np.nan, UFull) # Y + 2
        V_masked = np.where(X**2 + (Y + 2 * np.pi - 2 * delta_layer - rad)**2 <= radius**2, np.nan, VFull) # Y + 2
    ...
    fields = [
        (N_net, 'N_net', [-0.5, 0.5], f'img/electrode/n_net/neumann_close_large_domain_rad_1_stokes.png'),
        (Np,    'N_p',   [0,  2], f'img/electrode/n_p/neumann_close_large_domain_rad_1_stokes.png'),
        (Nm,    'N_m',   [0,  2], f'img/electrode/n_m/neumann_close_large_domain_rad_1_stokes.png'),
    ]
...
fig = plt.figure()
ax = fig.add_subplot(111, projection="3d")
ax.plot_surface(X[:-1,1:-1], Y[:-1,1:-1], body_interpolated_x, cmap=cmap, edgecolor='none')
...
fig = plt.figure()
ax = fig.add_subplot(111, projection="3d")
ax.plot_surface(X[1:-1,:-1], Y[1:-1,:-1], body_interpolated_y, cmap=cmap, edgecolor='none')
...
# Save results
savemat('electrode_close_particle_large_domain_rad_1_stokes.mat', {
    'ctxt_Rphi': ctxt,
    'u_next': u_next,
    'Xint': Xint,
    'Yint': Yint,
