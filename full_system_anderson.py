...
Phi_full[1:-1, 1:-1] = Phi_init
Phi_full[1:-1, 0] = Phi_init[:, -1]
Phi_full[1:-1, -1] = Phi_init[:, -1]
Phi_full[0, :] = -25
Phi_full[-1, :] = 25
Np_full[1:-1, 1:-1] = N_p_init
Nm_full[1:-1, 1:-1] = N_m_init
...
bodyForces_x = (Np_relevant_x.ravel(order='F') - Nm_relevant_x.ravel(order='F')) / 2 * Grad_x_Phi_Flat#(2 * delta_layer**2) * Grad_x_Phi_Flat
bodyForces_y = (Np_relevant_y.ravel(order='F') - Nm_relevant_y.ravel(order='F')) / 2 * Grad_y_Phi_Flat#(2 * delta_layer**2) * Grad_y_Phi_Flat
...
def solve_full_system(rhs_prev_rho, rhs_prev_F, U, Sigma, Vh, dLap, Nx, Ny, Nib, rad, delta_layer, tol, Sop_prime):
    computedRHS = cpeo.schur_rhs_R(dLap, rhs_prev_rho, Nx, Ny, Nib, delta_layer, Jop_prime)
    p = cpeo.solve_from_svd(U, Sigma, Vh, computedRHS)
    phi_tilde = cpeo.post_processing_compute_R(dLap, p, rhs_prev_rho, Nx, Ny, Nib, delta_layer, Sop_prime)

    offset = 0
    f_bc = rhs_prev_F[:(Nx * (Ny + 1))]
    offset += Nx * (Ny + 1)
    g_bc = rhs_prev_F[offset:offset + ((Nx + 1) * Ny)]
    offset += (Nx + 1) * Ny
    h_bc = rhs_prev_F[offset:offset + ((Nx + 1) * (Ny + 1))]
    offset += (Nx + 1) * (Ny + 1)
    z_x = rhs_prev_F[offset:offset + Nib]
    offset += Nib
    z_y = rhs_prev_F[offset:]

    U, V, P, lam_X, lam_Y = stokes.solve(-L/2, L/2, f_bc, g_bc, h_bc, z_x, z_y, rad, Nx, tol)

    return phi_tilde, np.concatenate([U, V, P, lam_X, lam_Y])

# Build dense Schur matrix
rhs_rho = b_Op_Schur(ctxt, U_fluid, V_fluid)
...
# Compute SVD once
U_SVD, Sigma, Vh_SVD = np.linalg.svd(schurDense)
...
rhs_full = np.concatenate([rhs_rho, rhs_F])

DU = np.full((len(rhs_full), m), np.nan)
DG = np.full((len(rhs_full), m), np.nan)

u_n = np.concatenate([np.zeros(len(rhs_F)), ctxt])
phi_solved, u_solved = solve_full_system(rhs_rho, rhs_F, U_SVD, Sigma, Vh_SVD, dLap, Nx, Ny, Nib, rad, delta_layer, tol, Sop_prime)
