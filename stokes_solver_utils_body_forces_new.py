...
def schur_rhs_new_ryker_trick(rhs, dLap_U, dLap_V, lu_factorization,
                  UGridX, UGridY, VGridX, VGridY,
                  delta_x, delta_y, xib, yib,
                  N_U, N_V, N_P, Nib, cut, Nx, dx):
    ...
    G_x_staggered, G_y_staggered, D_x_staggered, D_y_staggered = build_staggered_Grads_Divs(Nx, dx)

    # apply L^{-1} to RHS
    U_rhs = dLap_U.solve_A(rhs_U)
    V_rhs = dLap_V.solve_A(rhs_V)

    # compute first term of PL^{-1}
    PL_1 = np.concatenate([U_rhs, V_rhs])

    # compute second term of PL^{-1}
    D_rhs = D_x_staggered @ U_rhs + D_y_staggered @ V_rhs

    DG = D_x_staggered @ G_x_staggered + D_y_staggered @ G_y_staggered

    DG_D_Rhs = spsolve(DG, D_rhs)

    PL_2 = np.concatenate([G_x_staggered @ DG_D_Rhs, G_y_staggered @ DG_D_Rhs])
...
def schur_rhs_new_body_forces(rhs, dLap_U, dLap_V,
                  UGridX, UGridY, VGridX, VGridY,
                  delta_x, delta_y, xib, yib,
                  N_U, N_V, N_P, Nib, cut):
    ...
    # STEP 1: Apply inverse laplacian to (rhs_U, rhs_V)
    U = dLap_U.solve_A(rhs_U)
    V = dLap_V.solve_A(rhs_V)
...
def solve_factorized(left_bound, right_bound, lu_factorization, dLap_U, dLap_V, U_schur, Sigma_schur, Vh_schur, f_bc, g_bc, h_bc, z_x, z_y, rad, Nx, dx, cut):
    x = np.linspace(left_bound, right_bound, Nx + 2)
    dx = x[1] - x[0]
    y = x.copy()
    dy = dx
    ...
    RHS = np.concatenate([f_bc, g_bc, h_bc, z_x, z_y])
    RHS_schur_old = schur_rhs_new(RHS, lu_factorization, UGridX, UGridY, VGridX, VGridY, delta_x, delta_y, xib, yib, N_U, N_V, N_P, Nib, cut)
    RHS_schur = schur_rhs_new_body_forces(RHS, dLap_U, dLap_V, UGridX, UGridY, VGridX, VGridY, delta_x, delta_y, xib, yib, N_U, N_V, N_P, Nib, cut)
    RHS_schur_ryker = schur_rhs_new_ryker_trick(RHS, dLap_U, dLap_V, lu_factorization, UGridX, UGridY, VGridX, VGridY, delta_x, delta_y, xib, yib, N_U, N_V, N_P, Nib, cut, Nx, dx)


    # RHS_1_old = RHS_schur_old[:Nib]
    # RHS_2_old = RHS_schur_old[Nib:]
    # RHS_1_new = RHS_schur_ryker[:Nib]
    # RHS_2_new = RHS_schur_ryker[Nib:]
    # RHS_1_diff = (RHS_1_new - RHS_1_old) / np.mean(RHS_1_old)
    # RHS_2_diff = (RHS_2_new - RHS_2_old) / np.mean(RHS_1_old)

    # # compute error
    # error_x = np.linalg.norm(RHS_1_old - RHS_1_new) / np.linalg.norm(RHS_1_old)
    # error_y = np.linalg.norm(RHS_2_old - RHS_2_new) / np.linalg.norm(RHS_2_old)

    # print("Error x: ", error_x)
    # print("Error y: ", error_y)

    # # Create a line plot for individual points
    # # plt.plot(range(Nib), RHS_1_old, color='green', label='Old RHS x')
    # # plt.plot(range(Nib), RHS_2_old, color='red', label='Old RHS y')
    # # plt.plot(range(Nib), RHS_1_new, color='blue', label='New RHS x')
    # # plt.plot(range(Nib), RHS_2_new, color='cyan', label='New RHS y')

    # plt.plot(range(Nib), RHS_1_diff, color='green', label='Diff RHS x')
    # plt.plot(range(Nib), RHS_2_diff, color='red', label='Diff RHS y')
    ...
    # Solve
    sol = solve_from_svd(U_schur, Sigma_schur, Vh_schur, RHS_schur_ryker)
