...
Phi_BC[-1,:] = -2 * beta_BC * dy
N_p_BC = Npm_exact(X, Y)
N_p_BC[0,:] = -Np_electrode(np.zeros_like(x), np.zeros_like(x))
N_p_BC[-1,:] = -np.ones_like(x)
N_m_BC = Npm_exact(X, Y)
N_m_BC[0,:] = -Nm_electrode(np.zeros_like(x), np.zeros_like(x))
...
def b_Op_Schur(ctxt, bcs, phibc, npbc, nmbc, U_fluid, V_fluid): 
    return cpeo.Build_RHS_Schur_System_neumann(ctxt, bcs, phibc, npbc, nmbc, U_fluid, V_fluid, Lap_npm, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, Nib, Jop, Jop_prime, dx)

def b_Op(ctxt, bcs, phibc, npbc, nmbc, U_fluid, V_fluid):
    return cpeo.Build_RHS_rho_neumann(ctxt, bcs, phibc, npbc, nmbc, U_fluid, V_fluid, Lap_phi, Lap_npm, G_d_G_p, G_d_G_m, delta_layer, Nx, Ny, Nib, Jop, Jop_prime, dx)
...
for t_step in range(N_t):
    ...
    for inner_its in range(100000):
        ...
        G_u_next = cpeo.post_processing_compute_R_neumann(Lap_phi, Lap_npm, p_n, schurRHS, Nx, Ny, Nib, delta_layer, Sop_prime)
        ...
        # Check convergence
        res1, res2, res3 = cpeo.apply_Schur_R_neumann(Lap_phi, Lap_npm, [p, p_p, p_m], delta_layer, Nx, Ny, Sop_prime, Jop_prime)
        ...
        Phi_BC[-1,:] = -2 * beta_BC * dy
        N_p_BC = Npm_exact(X, Y)
        N_p_BC[0,1:-1] = -Np_electrode(Phi[1,:], Np[1,:])
        N_p_BC[-1,1:-1] = -np.ones_like(xint)
        N_m_BC = Npm_exact(X, Y)
        N_m_BC[0,1:-1] = -Nm_electrode(Phi[1,:], Nm[1,:])
