...
ld = loadmat('ellipsoid_results.mat')
...
ctxt = np.concatenate([
    ...
    np.zeros_like(N_p_init.ravel(order='F')),
    np.zeros_like(N_m_init.ravel(order='F')),
    np.zeros_like(xib),
    np.zeros_like(xib),
    np.zeros_like(xib)
    #Q_init,
...
# Apply to velocity fields
Np_masked = np.where(mask_top_int | mask_bot_int, np.nan, Np)
...
# stream = ax.streamplot(X, Y, U_masked, V_masked, color='black', density=2, linewidth=1, arrowsize=1.5, zorder=1)
